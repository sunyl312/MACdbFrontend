            ...
            <v-row>
                      ...
                      >
                        ...
                        <v-tab-item>
                          ...
                          <v-sheet class="pa-3 overflow-y-auto overflow-x-auto" height="750"
                            ...
                            <iframe
                                :src="'https://pubchem.ncbi.nlm.nih.gov/compound/'+compoundData.cid+'#section=3D-Conformer&embed=true&hide_title=true'"
                ...
                <v-row>
                  <v-col class="ml-6" cols="6">
                    <v-row>
                      ...
                      <v-col cols="12">
                        <v-sheet class="overflow-y-auto pr-8" height="300">
                          <v-row align="center">
              ...
              <v-col cols="12">
                <v-row>
                  ...
                  <v-col class="ml-6" cols="6">
                    <v-row>
                      <v-col :class="styles.formalHead" cols="6">Classification (Category)
                        <help-msg>This section shows the classfication of the metabolite. The larger the serial number,
                          the more accurate. Data are retrieved from PubChem website ( Classification > MeSH ).
                        </help-msg>
                      </v-col>
                      <v-col class="px-16" cols="12">
                        <v-simple-table fixed-header height="300">
                          ...
                          <tbody v-if="pubchemClassificationData &&  pubchemClassificationData.class_tree && (pubchemClassificationData.class_tree.length != 0)"
                                 class="text-left">
                          <tr v-for="(item,idx) in pubchemClassificationData && pubchemClassificationData.class_tree || []"
                              :key="idx">
                            <td class="my-auto">
                              <div>
<!--/*                                <span style="display: inline-block"><v-icon style="display: inline-block">mdi-arrow-down-thin</v-icon></span>*/-->
                                <span style="display: inline-block">{{ idx + 1 }}</span>
                              </div>


                            </td>
                            <td style="min-width: 250px"><a :href="item.url" style="text-decoration: none"
                                                            target="_blank">{{ item.term }}</a></td>
                            <td>
                          ...
                          <tbody
                              v-if="pubchemClassificationData &&  pubchemClassificationData.class_tree && (pubchemClassificationData.class_tree.length == 0) || false">
                          <tr>
                            <td colspan="3">
                  ...
                  <v-col class="ml-n6" cols="6">
                    <v-row>
                      <v-col :class="styles.formalHead" cols="6">Interacted Genes
                        <help-msg>This section shows the possible interactions between the metabolite and genes. Data
                          are retrieved from PubChem website ( DGidb ).
                        </help-msg>
                      </v-col>
                      <v-col class="px-16" cols="12">
                        <v-simple-table fixed-header height="300">
                          ...
                          <tbody v-if="pubchemDGIdbData && (pubchemDGIdbData.length != 0)" class="text-left">
                          <tr v-for="(item,idx) in pubchemDGIdbData && pubchemDGIdbData || []" :key="idx">
                            <td> {{ idx + 1 }}</td>
                            <td> {{ item.drugchemblid }}</td>
                            <td> {{ item.drugclaimname }}</td>
                            <td>{{ item.genename }}</td>
                            <td><a :href="'https://www.ncbi.nlm.nih.gov/gene/'+item.geneid" style="text-decoration: none"
                                   target="_blank">{{ item.geneid }}</a></td>
                            <td>
